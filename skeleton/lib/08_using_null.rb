...
def teachers_and_divisions
  ...
  execute(<<-SQL)
    SELECT 
      t.name, CASE WHEN dept_id in ('1', '2') THEN 'Sci' ELSE 'Art' END AS dept_category
    FROM 
      teachers t
  SQL
end
...
def teachers_and_divisions_two
  ...
  execute(<<-SQL)
    SELECT 
      t.name, CASE WHEN dept_id in ('1', '2') THEN 'Sci' WHEN dept_id in ('3') THEN 'Art' ELSE 'None' END AS dept_category
    FROM 
      teachers t
  SQL
end
