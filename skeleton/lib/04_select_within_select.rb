...
def sparse_continents
  ...
  execute(<<-SQL)
    ...
    WHERE 
      continent IN 
    (SELECT continent from (SELECT 
      continent, count(distinct case when population < 25000000 then name else null end) AS n_small, count(*) as n_total
    FROM 
      countries 
    GROUP BY 1) c WHERE n_small = n_total)
